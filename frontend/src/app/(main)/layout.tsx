...
import NotificationPanelContent from "@/components/notifications/NotificationPanelContent";
...
export default function MainLayout({ children }: SidebarProps) {
  ...
  const { remainingLabel: tempRemaining } = useTempCountdown(
  ...
  const unreadChatCount = useMemo(() => {
    return conversations.reduce((total, conversation) => {
      const unread =
        unreadCounts[conversation.id] ?? conversation.unreadCount ?? 0;
      return total + unread;
    }, 0);
        ...
        >
          <TooltipProvider>
              ...
              <div className="px-3 py-3">
                <div className="rounded-xl border border-border/60 bg-background/70 p-3">
                  <div ref={statusMenuRef}>
                    <div className="flex items-center gap-3 max-[456px]:flex-col">
                      ...
                      <div className="items-center ml-auto max-[456px]:w-full max-[456px]:ml-0">
            ...
            <div className="space-y-1 border-t border-border/50 px-2 py-2">
              ...
              {user?.role &&
                ["super_admin", "moderator", "support_staff"].includes(
                  user.role,
                ) && (
                  <Tooltip>
                    <TooltipTrigger asChild>
          ...
          <header className="relative z-20 flex h-14 items-center justify-between border-b border-border/60 bg-background/75 px-4 backdrop-blur-sm">
            <div className="flex items-center gap-3">
              ...
              <div className="relative z-50" ref={notificationMenuRef}>
                  ...
                  createPortal(
                    ...
                    >
                      <div
                        ...
                        className="mt-14 w-[min(92vw,24rem)] max-h-[min(60vh,24rem)] rounded-xl border border-border/70 bg-card/95 p-2 shadow-elevated backdrop-blur-sm animate-slide-down"
                ...
                {notificationMenuOpen && !isMobile && (
                  <div
                    ref={notificationMenuRef}
                    className="absolute left-0 top-full z-50 mt-2 w-[min(92vw,24rem)] rounded-xl border border-border/70 bg-card/95 p-2 shadow-elevated backdrop-blur-sm"
                  >
                    <NotificationPanelContent
                      notifications={notifications}
                      unreadCount={unreadCount}
