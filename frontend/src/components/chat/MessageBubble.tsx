...
interface MessageBubbleProps {
  ...
  readByUsersById?: Record<
    string,
    { name?: string; anonimiId?: string; profileImage?: string | null }
  >;
...
export default function MessageBubble({
  ...
  const isEdited = Boolean(
    message.editedAt || (message.editHistory?.length ?? 0) > 0,
  );

  const stealthExpiresAt = message.stealthExpiresAt
    ? new Date(message.stealthExpiresAt).getTime()
    : null;
  const stealthExpired =
    isStealth &&
    (!!message.stealthExpiredAt ||
      (stealthExpiresAt ? stealthExpiresAt <= now : true));
  const remainingMs = stealthExpiresAt
    ? Math.max(stealthExpiresAt - now, 0)
    : 0;
  ...
  async function handleSendMessage() {
    ...
    try {
      const res = await api.post("/conversations", {
        participantAnonimiId: senderAnonimiId,
      });
      const conversationId = res.data?.data?.conversationId as
        | string
        | undefined;
      if (!conversationId) {
  ...
  return (
    <>
      <div
        ...
        onMouseLeave={() => {
          if (!dialogOpen) setShowActions(false);
        }}
      >
        {/* Avatar */}
        {!isMine && (
        ...
        <div className={cn("flex flex-col min-w-0 max-w-[78%] md:max-w-[70%]", isMine && "items-end")}>
          ...
          <div className="flex min-w-0 gap-2 relative">
            ...
            >
              ...
              {message.failed && (
                <span className="text-xs text-destructive mt-1 block text-right">
                  Failed
                </span>
              )}
            </div>
